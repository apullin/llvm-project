...
// RUN: %S/../../../../../i8085-trace/build/i8085-trace -S -q -n 200000 -d %testdump_addr:0x4 %t.bin 2>&1 | FileCheck %s --check-prefix=F32
...
// RUN: %S/../../../../../i8085-trace/build/i8085-trace -S -q -n 200000 -d %testdump_addr:0x4 %t.o1.bin 2>&1 | FileCheck %s --check-prefix=F32
...
// RUN: %S/../../../../../i8085-trace/build/i8085-trace -S -q -n 200000 -d %testdump_addr:0x4 %t.os.bin 2>&1 | FileCheck %s --check-prefix=F32
...
// RUN: %S/../../../../../i8085-trace/build/i8085-trace -S -q -n 200000 -d %testdump_addr:0x4 %t.o2.bin 2>&1 | FileCheck %s --check-prefix=F32
...
int main(void) {
  ...
  out[0] = a + b; // 3.75f -> 0x40700000
  return 0;
}

// F32: Memory dump {{0x[0-9A-F][0-9A-F][0-9A-F][0-9A-F]}} - {{0x[0-9A-F][0-9A-F][0-9A-F][0-9A-F]}} (4 bytes):
// F32: {{ *}}{{[0-9A-F][0-9A-F][0-9A-F][0-9A-F]}}:{{ *}}00 00 70 40
// F32: "halt":"hlt"
