...
// RUN: %S/../../../../../i8085-trace/build/i8085-trace -S -q -n 200000 -d %testdump_addr:0x20 %t.bin 2>&1 | FileCheck %s --check-prefix=U32CMP
...
// RUN: %S/../../../../../i8085-trace/build/i8085-trace -S -q -n 200000 -d %testdump_addr:0x20 %t.o1.bin 2>&1 | FileCheck %s --check-prefix=U32CMP
...
// RUN: %S/../../../../../i8085-trace/build/i8085-trace -S -q -n 200000 -d %testdump_addr:0x20 %t.os.bin 2>&1 | FileCheck %s --check-prefix=U32CMP
...
// RUN: %S/../../../../../i8085-trace/build/i8085-trace -S -q -n 200000 -d %testdump_addr:0x20 %t.o2.bin 2>&1 | FileCheck %s --check-prefix=U32CMP
...
volatile uint32_t a = 0x3FC00000u;  // 1.5f bits
volatile uint32_t b = 0x40100000u;  // 2.25f bits
volatile uint32_t inf = 0x7F800000u;
volatile uint32_t small_a = 1;
volatile uint32_t small_b = 2;
...
int main(void) {
  ...
  out[4] = (a < b);                // expect 1
  out[5] = (b > a);                // expect 1
  out[6] = (small_a < small_b);    // expect 1
  out[7] = (small_b < small_a);    // expect 0
  return 0;
}

// U32CMP: Memory dump {{0x[0-9A-F][0-9A-F][0-9A-F][0-9A-F]}} - {{0x[0-9A-F][0-9A-F][0-9A-F][0-9A-F]}} (32 bytes):
// U32CMP: {{ *}}{{[0-9A-F][0-9A-F][0-9A-F][0-9A-F]}}:{{ *}}00 00 00 00 01 00 00 00 01 00 00 00 00 00 00 00
// U32CMP: {{ *}}{{[0-9A-F][0-9A-F][0-9A-F][0-9A-F]}}:{{ *}}01 00 00 00 01 00 00 00 01 00 00 00 00 00 00 00
// U32CMP: "halt":"hlt"
