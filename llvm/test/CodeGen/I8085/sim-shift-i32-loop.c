...
// RUN: %S/../../../../../i8085-trace/build/i8085-trace -S -q -n 2000000 -d %testdump_addr:0x8 %t.bin 2>&1 | FileCheck %s --check-prefix=SHI32
...
int main(void) {
  int32_t pos = 1;
  int32_t neg = (int32_t)0x80000001;
  uint32_t acc_l = 0;
  uint32_t acc_r = 0;
  for (uint8_t s = 0; s < 32; ++s) {
    acc_l ^= (uint32_t)(pos << s);
    acc_r ^= (uint32_t)(neg >> s);
  }
  out[0] = acc_l;
  out[1] = acc_r;
  return 0;
...
// SHI32: Memory dump {{0x[0-9A-F][0-9A-F][0-9A-F][0-9A-F]}} - {{0x[0-9A-F][0-9A-F][0-9A-F][0-9A-F]}} (8 bytes):
// SHI32: {{ *}}{{[0-9A-F][0-9A-F][0-9A-F][0-9A-F]}}:{{ *}}FF FF FF FF 54 55 55 55
// SHI32: "halt":"hlt"
