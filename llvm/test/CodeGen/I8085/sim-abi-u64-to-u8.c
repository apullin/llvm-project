...
// RUN: %S/../../../../../i8085-trace/build/i8085-trace -S -q -n 200000 -d 0x0200:0x1 %t.bin 2>&1 | FileCheck %s --check-prefix=ABI

#include <stdint.h>

static volatile uint64_t in_val = 0x0123456789ABCDEFULL;

__attribute__((noinline)) static uint8_t conv(uint64_t x) {
  return (uint8_t)x;
}

int main(void) {
  volatile uint8_t *p = (volatile uint8_t *)0x0200;
  volatile uint8_t v = conv(in_val);
  uint8_t u = (uint8_t)v;
  p[0] = (uint8_t)(u >> 0);
  return 0;
}

// ABI: Memory dump 0x0200 - 0x0200 (1 bytes):
// ABI: 0200: EF
// ABI: "halt":"hlt"
